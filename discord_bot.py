...
LOG_FILES = {
    ...
    "CLAUDE": AI_DIR / "logs" / "claude.log",
    "GEMINI": AI_DIR / "logs" / "gemini.log",
    "REVIEWER": AI_DIR / "logs" / "review.log",
}
STATUS_FILE = AI_DIR / "TEAM_STATUS.md"
OUTBOX_HUMAN = AI_DIR / "messages" / "outbox_human"
INBOX_CLAUDE = AI_DIR / "messages" / "inbox_claude"
INBOX_GEMINI = AI_DIR / "messages" / "inbox_gemini"
...
CLAUDE_REVIEW_DISABLED_SENTINEL = AI_DIR / "messages" / "DISABLE_CLAUDE_REVIEW"
...
# --- EMBED COLORS ---
COLOR_CLAUDE = discord.Color.from_str("#5865F2")  # blurple
COLOR_GEMINI = discord.Color.from_str("#10B981")  # teal
...
def _write_task(task_text: str, target: str, skip_review: bool) -> str:
...
async def watch_tasks():
    ...
    try:
        ...
        for inbox, label, color in [
            (INBOX_CLAUDE, "CLAUDE", COLOR_CLAUDE),
            (INBOX_GEMINI, "GEMINI", COLOR_GEMINI),
        ]:
            if not inbox.exists():
                continue
...
async def on_ready():
    ...
    if logs_channel:
        await logs_channel.send(
            ...
            f"Watching: orchestrator · claude · gemini")
...
async def cmd_dcr(ctx):
    """Toggle Claude's UI/UX review of Gemini's frontend diffs on or off.
    Usage: !dcr"""
...
async def cmd_task(ctx):
    ...
    async def _reject():
        ...
        try:
            await ctx.author.send(
                f"the message has been rejected (check routing flag: -c / -g)")
            logger.info(
    ...
    CLAUDE_FLAGS = ("-c", "--c", "--claude")
    GEMINI_FLAGS = ("-g", "--g", "--gemini")
    ...
    review_note = "  *(code review skipped)*" if skip_review else ""
    confirm_embed = discord.Embed(
        title="⚠️  Confirm Task Submission",
        description=_truncate(task_text, 1800),
        color=COLOR_CLAUDE if target == "claude" else COLOR_GEMINI,
    )
    ...
    # Write the task file
    try:
        filename = _write_task(task_text, target, skip_review)
    except Exception as e:
...
async def cmd_task_error(ctx, error):
    ...
    try:
        await ctx.author.send(
            "the message has been rejected (check routing flag: -c / -g)")
    except Exception:
        pass
...
async def cmd_tasks(ctx):
    ...
    try:
        rows = []
        for label, path in [("Claude inbox", INBOX_CLAUDE),
                            ("Gemini inbox", INBOX_GEMINI),
                            ("Human inbox",
                             AI_DIR / "messages" / "inbox_tasks"),
