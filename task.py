...
def create_task(task_text, target, skip_review=False, skip_ui_review=False):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        if skip_ui_review:
            f.write(f"Skip-Claude-UI-Review: true\n")
        f.write(f"---\n\n")
        f.write(task_text)
    ...
    if skip_ui_review:
        notes.append("Claude UI review skipped")
...
if __name__ == "__main__":
    ...
    if len(args) < 1:
        print(
            "Usage: python ai_team/task.py ['Your task here'] [-c|--claude | -g|--gemini] [-nr|--no-review] [-dcr|--disable-claude-review]"
        )
        print(
            "       python ai_team/task.py -a [optional/path/to/file]  (audit mode)"
        )
        print("       Default agent: Claude (omit flag to send to Claude)")
        print(
            "       -dcr skips Claude's UI/UX review of Gemini's output for this task"
        )
        sys.exit(1)
    ...
    # Extract -dcr / --disable-claude-review flag (position-independent)
    no_ui_review = "-dcr" in args or "--disable-claude-review" in args
    args = [a for a in args if a not in ("-dcr", "--disable-claude-review")]
    ...
    if args[0].lower() in ["-c", "--claude"]:
        target_agent = "claude"
        if len(args) < 2:
            print("❌ Error: No task text provided after routing flag.")
            sys.exit(1)
        task_text = args[1]
    elif args[0].lower() in ["-g", "--gemini"]:
        target_agent = "gemini"
        if len(args) < 2:
            print("❌ Error: No task text provided after routing flag.")
            sys.exit(1)
